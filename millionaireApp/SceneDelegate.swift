...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if UserDefaultsHelper.getBool(key: "test") {
            ...
            do {
                if let userData1: Users = try UserDefaultsHelper.getCustom(key: "testCodable") {
                    vc.setUser(userData1)
                    print(userData1)
                } else {
                    // Handle the case where decoding returned nil
